...
if __name__ == "__main__":
    ...
    print("clean mesh ....")
    cmd = f"python {script_dir}/clean_mesh.py --exp_dir {os.path.dirname(ply_file)} --scan {scan} --mesh_name {os.path.basename(ply_file)} --mask_dir {args.mask_dir}"
    os.system(cmd)

    print("cull mesh ....")
    result_mesh_file = os.path.join(out_dir, "culled_mesh.ply")
    cull_scan(scan, os.path.join(os.path.dirname(ply_file), f'new_{scan}.ply'), result_mesh_file, instance_dir=os.path.join(args.mask_dir, f'scan{scan}'))
