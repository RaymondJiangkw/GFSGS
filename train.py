...
from arguments import ModelParams, PipelineParams, SHOptimizationParams, LatentOptimizationParams
try:
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if not viewpoint_stack:
            ...
            if iteration > opt.geovalue_reset_interval and iteration < opt.densify_until_iter: # iteration % opt.geovalue_reset_interval > len(scene.getTrainCameras()):
                ...
                # if opt.use_weight_cull:
                #     gaussians.prune_by_weight(opt.cull_min_weight)
                #     gaussians.compute_adjacent_matrix()
        ...
        if iteration <= 7000 and dataset.color_mode != 'sh':
        ...
        with torch.no_grad():
            ...
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), test_interval, scene, render, (pipe, background))
            if (iteration in saving_iterations):
                print("\n[ITER {}] Saving Gaussians".format(iteration))
                scene.save(iteration)

            gaussians.count_accum += render_pkg['count_accum']
            gaussians.max_weight = torch.max(gaussians.max_weight, render_pkg['weight_accum'])
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    size_threshold = 20 if iteration > opt.geovalue_reset_interval else None
                    gaussians.densify_and_prune(opt.densify_grad_threshold, opt.densify_absgrad_threshold, opt.geovalue_cull, scene.cameras_extent, size_threshold, percent_dense=opt.percent_dense, use_absgrad=iteration >= opt.densify_absgrad_start_iteration)
                    gaussians.compute_adjacent_matrix()
                
                if iteration < opt.geovalue_reset_until_iter and (iteration % opt.geovalue_reset_interval == 0 or (dataset.white_background and iteration == opt.densify_from_iter)):
                    gaussians.reset_geovalue(opt.geovalue_reset)
            ...
            if iteration < opt.iterations:
                # Clamp gradient
                for name in ['_xyz', '_features_dc', '_features_rest', '_geovalue', '_scaling', '_rotation']:
                    if getattr(gaussians, name).grad is None:
                        continue
                    getattr(gaussians, name).grad = torch.nan_to_num(getattr(gaussians, name).grad, nan=0, posinf=1e5, neginf=-1e5)
            ...
            with torch.no_grad():
                ...
                while network_gui.conn != None:
                    try:
                        ...
                        metrics_dict = {
                            "#": gaussians.get_geovalue.shape[0],
...
@torch.no_grad()
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, test_interval, scene : Scene, renderFunc, renderArgs):
    if tb_writer:
...
if __name__ == "__main__":
    ...
    if 'sh' in sys.argv[1:]:
        print('Use SH Representation for Color...')
        op = SHOptimizationParams(parser)
    else:
        # Default is Latent
        print('Use Latent Representation for Color...')
        op = LatentOptimizationParams(parser)
    pp = PipelineParams(parser)
    parser.add_argument('--ip', type=str, default="127.0.0.1")
    parser.add_argument('--port', type=int, default=6009)
    parser.add_argument('--detect_anomaly', action='store_true', default=False)
    parser.add_argument("--test_interval", type=int, default=1_000)
